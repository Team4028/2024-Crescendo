...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        driverController.x().and(driverController.povRight())
                .toggleOnTrue(Commands.runOnce(() -> {
                    ...
                    pivot.runPivotToPosition(entry.Angle);
                }, shooter, pivot));
        ...
        driverController.y().and(driverController.povDown())
                .onTrue(climber.runToPositionCommand(Climber.ClimberPositions.HOME));

        /* Run Climber to "Catch" */
        driverController.y().and(driverController.povLeft().or(driverController.povRight()))
                .onTrue(climber.runToPositionCommand(Climber.ClimberPositions.CATCH));

        /* Run Climber to "Ready" */
        driverController.y().and(driverController.povUp())
                .onTrue(climber.runToPositionCommand(Climber.ClimberPositions.READY));
        ...
        driverController.rightBumper().onTrue(pivot.runPivotCommand(PIVOT_VBUS))
                .onFalse(pivot.runPivotCommand(0.0));
        driverController.leftBumper().onTrue(pivot.runPivotCommand(-PIVOT_VBUS))
                .onFalse(pivot.runPivotCommand(0.0));
    ...
    public Command getAutonomousCommand() {
        return new InstantCommand(() -> drivetrain.seedFieldRelative(new Pose2d()))
                .alongWith(pivot.pivotZeroCommand()).andThen(autonChooser.getSelected());
    }

    /* Zeroing Command */
    public Command zeroCommand() {
        return climber.zeroCommand().andThen(pivot.pivotZeroCommand());
    }
