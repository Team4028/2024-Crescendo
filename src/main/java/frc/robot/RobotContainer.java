...
public class RobotContainer {
        ...
        private static final double SMART_INFEED_IN_TIMEOUT = 0.1;
        private static final double SMART_INFEED_OUT_TIMEOUT = 0.5;
        private static final double SMART_INFEED_FINAL_TIMEOUT = 0.1;
        ...
        public RobotContainer() {
                ...
                smartInfeedCommand = infeed.runInfeedMotorCommand(INFEED_VBUS)
                                ...
                                .andThen(infeed.runInfeedMotorCommand(0.).alongWith(conveyor.runMotorCommand(0.))
                                                .repeatedly().withTimeout(SMART_INFEED_IN_TIMEOUT))
                                .andThen(shooter.spinMotorRightCommand(SHOOTER_BACKOUT_VBUS).repeatedly()
                                                .raceWith(conveyor.runXRotations(-1.)
                                                                .withTimeout(SMART_INFEED_OUT_TIMEOUT)
                                                                .alongWith(infeed.runInfeedMotorCommand(0.))))
                                .andThen(shooter.spinMotorRightCommand(0.).withTimeout(SMART_INFEED_FINAL_TIMEOUT));
